...
红色草药问答系统是一个创新性的移动端学习平台，将中国共产党的红色革命精神与传统中医草药知识深度融合。通过真实的历史故事和草药应用场景，让用户在学习中医药文化的同时，感悟革命先辈在艰苦环境下因地制宜、自力更生的伟大精神。

### 🌟 项目特色

- **红色主题**: 每道题目都关联具体的红色革命历史事件
- **双重学习**: 同时学习红色精神和中医草药知识
- **真实故事**: 基于历史文献和革命回忆录的真实场景
- **移动优先**: 专为手机用户设计的流畅体验
...
- **随机抽题**: 从题库中随机抽取红色草药题目
- **难度筛选**: 简单、中等、困难三个等级
- **精神分类**: 按红色精神分类（长征精神、延安精神、井冈山精神、抗美援朝精神等）
- **题目详情**: 题目文本、四个选项、正确答案、历史故事解析

### 3. 答题功能模块

- **答题提交**: 提交用户选择的答案
- **即时反馈**: 显示答案是否正确
- **详细解析**: 展示正确答案、草药知识和红色故事背景
- **统计更新**: 自动更新用户答题统计
...
| explanation    | TEXT         | 题目解析（含草药知识+红色故事） |
| difficulty     | ENUM         | 难度(easy/medium/hard)          |
| category       | VARCHAR(50)  | 分类（红色精神类别）            |
